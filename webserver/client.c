...
static void
client_send(int fd, char *filename)
{
	char buf[MAXLINE];
	char hostname[MAXLINE];

	SYS(gethostname(hostname, MAXLINE));
...
client_request(void *arg)
{
	...
	for (i = 0; i < cl->nr_times; i++) {
		...
		// cl->fileset[fnr].name);
		client_send(clientfd, cl->fileset[fnr].name);
...
usage(char *program)
{
	fprintf(stderr, "Usage: %s [-t] host port nr_times nr_threads filename\n",
		program);
	exit(1);
}
